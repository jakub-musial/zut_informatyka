...
struct some_object
{
	...
	some_object() {}
	some_object(int i, char c) : field_1(i), field_2(c) {}

	friend std::ostream& operator << (std::ostream& output, some_object& so_obj);
};
...
public:
	...
	void clear()//bool clear_data = false)
	{
		...
		while (curr_node != nullptr)
		{
			...
			//if (clear_data) delete(curr_node->data);
			delete(curr_node);
			curr_node = next;
	...
	void display()
	{
		...
		while (tmp != nullptr)
		{
			...
			std::cout << "Next node: " << tmp->next_node << std::endl << std::endl;
			tmp = tmp->next_node;
		}
...
bool operator == (some_object& left, some_object& right)
{
	if (left.field_1 == right.field_1 && left.field_2 == right.field_2) { return true; }
	else { return false; }
}

bool operator <= (some_object& left, some_object& right)
{
	if (left.field_1 <= right.field_1)      { return true;  }
	else if (left.field_2 <= right.field_2) { return true;  }
	else                                    { return false; }
}
...
std::ostream &operator << (std::ostream& output, some_object& so_obj)
{
	output << "\n" << "field_1: " << so_obj.field_1 << "\n" << "field_2: " << so_obj.field_2;
	return output;
}