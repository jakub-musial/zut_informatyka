...
template <typename T> DynamicArray<T>::DynamicArray(size_t s) : max_size(s), size(0), tab(new T[s]) {}
...
template <typename T> void DynamicArray<T>::insertion_sort()
{
	size_t i, j;
	float key;
	for (i = 1; i < size; i++)
	{
		...
		while (j < size && tab[j] > key)
		{
			//if (size >= 2) std::cout << tab[j + 1] << " " << tab[i] << std::endl;
			tab[j + 1] = tab[j];
			j = j - 1;
		}
		tab[j + 1] = key;
...
template <typename T> void DynamicArray<T>::set_size(size_t s)
{
	size = s;
}
